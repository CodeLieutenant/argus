...
@pytest.mark.docker_required
def test_register_tunnel_unauthenticated(flask_client: FlaskClient, argus_db, active_config, monkeypatch):
    """Requests without a valid auth token should be rejected with 403."""
    from flask import g
    monkeypatch.setattr("argus.backend.service.user.load_logged_in_user", lambda: setattr(g, "user", None) or None)
    # Use a fresh client without the session user

...
/ keyless -> inert, summarizer error -> no write, and the min-chars gate.
"""
...
def test_min_chars_gate_skips_short_events(monkeypatch):
    monkeypatch.setattr(sd, "Summarizer", FakeSummarizer)
    db = FakeDB(_config(EVENT_SUMMARIZATION_MIN_CHARS=800))
    dispatcher = sd.SummaryDispatcher(db, db.config)

    _dispatch_one(dispatcher, "short message")  # below threshold
    assert db.executed == []

    dispatcher2 = sd.SummaryDispatcher(db, db.config)
    _dispatch_one(dispatcher2, "x" * 900)  # above threshold
    assert len(db.executed) == 1
...
def test_malformed_config_disables_without_crashing(monkeypatch):
    monkeypatch.setattr(sd, "Summarizer", FakeSummarizer)
    db = FakeDB(_config(EVENT_SUMMARIZATION_MIN_CHARS="not-a-number"))

...
_ROUND = {
    "avg_coverage": 1, "avg_faithfulness": 1, "avg_conciseness": 1, "avg_overall": 1,
    "avg_compression": 3, "avg_cost": 6, "avg_latency_ms": 0, "avg_completion_tokens": 1,
}
...
def render_table(rows: list[dict], baseline: dict, tolerance: float) -> str:
    ...
    for r in rows:
        ...
        lines.append(
            f"{r['key']:<34}{r['faith']:>13}{r['cover']:>13}{r['concise']:>13}{r['cost']:>13}  {verdict}"
        )
    return "\n".join(lines)
